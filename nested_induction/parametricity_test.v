...
Definition printKername {A:Type} (indTm : A) : TemplateMonad unit
  := p <- tmQuoteRec indTm;;
     let (env,tm) := (p:Ast.program) in
     match tm with
     | Ast.tInd ind0 univ =>
       (* decl <- tmQuoteInductive (inductive_mind ind0) ;; *)
       tmPrint (inductive_mind ind0)
     | _ => tmPrint tm ;; tmFail " is not an inductive"
     end.

MetaCoq Run (printKername list).
...
(* MetaCoq Run (printKername rose). *)
(* MetaCoq Run (tmQuoteInductive (MPfile ["parametricity_test"], "rose") >>= tmPrint). *)
...
Print dirpath. *)
(* Search kername. *)
MetaCoq Run (tmQuoteInductive (MPfile [], "AllI") >>= tmPrint).
